...
async function getActiveSubmission(studentId) {
  const [rows] = await db.query(
    `SELECT id, request_number AS request_number, status
...
async function createSubmission(data) {
  ...
  try {
    ...
    // ── Generate nomor SK format resmi ───────────────────────────────────────
    // Format: [Nomor Urut]/UN16.15/KM/[Tahun]
    // UN16    = kode Universitas Andalas
    // .15     = kode Fakultas Teknologi Informasi
    // KM      = kode jenis surat (Keputusan Mahasiswa)
    // Nomor urut di-reset setiap tahun (dihitung dari DB)
    const year = new Date().getFullYear();
    const [[{ count }]] = await conn.query(
      `SELECT COUNT(*) AS count FROM student_requests
       WHERE request_type = 'resignation' AND YEAR(created_at) = ?`,
      [year]
    );
    const seq           = String(count + 1).padStart(3, '0');
    const requestNumber = `${seq}/UN16.15/KM/${year}`;
    ...
    const [reqResult] = await conn.query(
      `INSERT INTO student_requests
         (request_number, request_type, title, description, status, requested_by, created_at, updated_at)
...
async function getSubmissionsByStudentId(studentId, { search = '', page = 1, limit = 10 } = {}) {
  ...
  const searchParam = `%${search}%`;
  const [rows] = await db.query(
    `SELECT sr.id, sr.request_number AS request_number, sr.status,
     ...
     WHERE sr.requested_by = ? AND sr.request_type = 'resignation'
       AND (sr.request_number LIKE ? OR sr.description LIKE ?)
  ...
  const [[{ total }]] = await db.query(
    `SELECT COUNT(*) AS total FROM student_requests
     WHERE requested_by = ? AND request_type = 'resignation'
       AND (request_number LIKE ? OR description LIKE ?)`,
...
async function getAllSubmissions({ search = '', status = '', page = 1, limit = 10 } = {}) {
  ...
  if (search) {
    conditions.push(`(s.name LIKE ? OR s.regno LIKE ? OR sr.request_number LIKE ?)`);
...
async function getSubmissionById(id, studentId = null) {
  let sql = `SELECT sr.id, sr.request_number AS request_number, sr.status,
                    sr.requested_at, sr.created_at, sr.updated_at, sr.description AS reason,
                    sr.requested_by,
...
async function submitSubmission(id, studentId) {
  // Validasi kelengkapan semua data wajib sesuai ketentuan fitur §Validasi Pengajuan
  // sebelum mengizinkan submit. Mencegah Draft "kosong" diajukan.
  const [rows] = await db.query(
    `SELECT sr.status, sr.description AS reason,
            srr.application_letter_file, srr.student_address, srr.student_hp,
            srr.current_gpa, srr.current_credits
     FROM student_requests sr
     LEFT JOIN student_request_resignation srr ON srr.student_requests_id = sr.id
  ...
  const row = rows[0];
  const missing = [];
  if (!row.student_address || !String(row.student_address).trim()) missing.push('Alamat');
  if (!row.student_hp || !String(row.student_hp).trim()) missing.push('Nomor HP');
  if (row.current_gpa === null || row.current_gpa === undefined || row.current_gpa === '') missing.push('IPK Saat Ini');
  if (row.current_credits === null || row.current_credits === undefined || row.current_credits === '') missing.push('Jumlah SKS Saat Ini');
  if (!row.reason || !String(row.reason).trim()) missing.push('Alasan Pengunduran Diri');
  if (!row.application_letter_file) missing.push('Surat Permohonan (PDF)');
  if (missing.length > 0) {
    throw new Error(`Data belum lengkap, harap isi: ${missing.join(', ')}.`);
  }
